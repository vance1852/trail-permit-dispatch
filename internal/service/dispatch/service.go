...
func (s *Service) RegisterMembers(ctx context.Context, actor domain.Actor, code string, items []MemberInput) (BatchResult, error) {
	...
	for _, item := range items {
		...
		if err := s.registerOneMember(ctx, actor, party, item); err != nil {
...
func (s *Service) registerOneMember(ctx context.Context, actor domain.Actor, party *domain.Party, item MemberInput) error {
	...
	return s.tx.WithTx(ctx, func(txCtx context.Context) error {
		registered, err := s.parties.CountMembers(txCtx, party.ID)
		if err != nil {
			return err
		}
		if registered >= party.Size {
