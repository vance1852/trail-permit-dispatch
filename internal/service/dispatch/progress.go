...
func (s *Service) CompleteParty(ctx context.Context, actor domain.Actor, code string) (PartyView, error) {
	...
	// 名额归还不影响行程是否算完成，放在事务之外执行，避免归还失败把已经
	// 走完的行程一起回滚。
	if releaseErr := s.releasePermit(ctx, party, now); releaseErr != nil {
		logging.FromContext(ctx).Warn("完成行程后归还许可名额失败",
			"party", party.Code, "error", releaseErr.Error())
	}
	refreshed, err := s.parties.ByID(ctx, party.ID)
	if err != nil {
		return PartyView{}, err
