...
// ByTokenHash loads a usable session by the hash of its bearer token. Sessions
// that were explicitly revoked are filtered out here so callers only ever see
// credentials that may still authenticate a request.
func (r *SessionRepo) ByTokenHash(ctx context.Context, tokenHash string) (*domain.Session, error) {
	row := r.q(ctx).QueryRowContext(ctx,
		`SELECT `+sessionColumns+` FROM sessions WHERE token_hash = ? AND revoked_at IS NULL`, tokenHash)
