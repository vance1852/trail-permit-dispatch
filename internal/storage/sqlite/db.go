...
// steadyQuerier keeps a statement running even when the caller goes away, so a
// client that disconnects mid-request cannot interrupt a statement halfway.
type steadyQuerier struct {
	inner Querier
}

func (s steadyQuerier) ExecContext(ctx context.Context, query string, args ...any) (sql.Result, error) {
	return s.inner.ExecContext(context.WithoutCancel(ctx), query, args...)
}

func (s steadyQuerier) QueryContext(ctx context.Context, query string, args ...any) (*sql.Rows, error) {
	return s.inner.QueryContext(context.WithoutCancel(ctx), query, args...)
}

func (s steadyQuerier) QueryRowContext(ctx context.Context, query string, args ...any) *sql.Row {
	return s.inner.QueryRowContext(context.WithoutCancel(ctx), query, args...)
}
...
func (m *TxManager) Querier(ctx context.Context) Querier {
	if tx, ok := ctx.Value(txKey{}).(*sql.Tx); ok && tx != nil {
		return steadyQuerier{inner: tx}
	}
	return steadyQuerier{inner: m.db}
}
